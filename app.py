...
# + Feedback (댓글달기/문의하기) → GitHub Issues
...
def create_issue(repo_full: str, token: str, title: str, body: str):
    url = f\"https://api.github.com/repos/{repo_full}/issues\"
    r = requests.post(url, headers=_gh_headers(token), json={\"title\": title, \"body\": body})
    return r
...
def feedback_ui():
    ...
    if not gh_token or not gh_repo:
        st.info(\"**관리자 안내:** Streamlit Secrets에 `GH_TOKEN`, `GH_REPO`를 설정하면 여기서 접수된 내용이 GitHub Issues로 자동 저장됩니다.\\n\\n\"
        ...
        with st.form(\"form_praise\"):
            ...
            if submitted:
                if not subject or not msg:
                    ...
                    title = f\"[칭찬] {subject}\"  # 이슈 제목 접두사는 그대로 유지
                    body = f\"\"\"{msg}
...
**시간(UTC)**: {now}
\"\"\"
                    res = create_issue(gh_repo, gh_token, title, body)
...
\"\"\"
    ...
    with st.expander(\"최근 접수된 피드백 보기(최대 10개)\"):
        try:
            items = list_issues(gh_repo, gh_token, state=\"open\", per_page=10)
            if not items:
                st.write(\"표시할 항목이 없습니다.\")
            else:
                for it in items:
                    st.write(f\"• **#{it.get('number')}** {it.get('title')} — {it.get('user',{}).get('login','')}\")
        except Exception:
